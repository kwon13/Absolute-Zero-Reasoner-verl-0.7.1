...
class CodeIORayPPOTrainer(ReasonRLRayPPOTrainer):
    ...
    def _compute_batch(self, batch: DataProto, metrics: dict, timing_raw: dict, problem_type: str, executor: PythonExecutor) -> tuple[DataProto, dict]:
        ...
        # verl 0.7.1 AgentLoop needs raw_prompt in non_tensor_batch; also keep tensor key pop
        # so batch.union(gen_batch_output) doesn't conflict on input_ids/attention_mask/position_ids.
        _non_tensor_pop = [k for k in ('raw_prompt', 'raw_prompt_ids') if k in batch.non_tensor_batch]
        gen_batch = batch.pop(
            batch_keys=['input_ids', 'attention_mask', 'position_ids'],
            non_tensor_batch_keys=_non_tensor_pop,
        )

        # generate a batch
        # sync current actor weights into the (dummy-initialized) vLLM engine first
        ...
        with marked_timer(f'adv/{problem_type}', timing_raw):
            ...
            if problem_type.startswith('gen'):
                reward_fn_kwargs = {
                    ...
                    # verl 0.7.1: difficulty estimation must generate via the async AgentLoop,
                    # not the worker-group sync interface (worker event loop is busy).
        ...
        while problem_types != ['code_f']: # we can skip this loop if we are only generating code_f dataset
            ...
            for problem_type in problem_types:
                ...
                for batch_dict in seed_dataloader:
                    ...
                    gen_batch_padded, pad_size = pad_dataproto_to_divisor(gen_batch, self.config.actor_rollout_ref.rollout.agent.num_workers)
                    # sync current actor weights into the (dummy-initialized) vLLM engine first
                    self._sync_rollout_weights()
    ...
    def _sync_rollout_weights(self):
        """Push current FSDP actor weights into the vLLM rollout engine.

        verl 0.7.1 hybrid engine inits vLLM with dummy (random) weights
        (rollout.load_format=dummy) and relies on the trainer calling
        checkpoint_manager.update_weights() before each rollout. The stock
        verl PPO trainer does this in its fit() loop, but AZR's custom fit()
        calls async_rollout_manager.generate_sequences() directly and so
        skipped it -> vLLM generated from random weights -> gibberish ->
        0 valid programs -> 0 advantage -> 0 loss.

        Weights only change once per global_step (on actor update), so we sync
        at most once per step. naive checkpoint-engine backend makes this a
        simple FSDP->vLLM tensor copy.
        """
        if getattr(self, "checkpoint_manager", None) is None:
            return
        if getattr(self, "_last_weight_sync_step", None) == self.global_steps:
            return
        self.checkpoint_manager.update_weights(self.global_steps)
        self._last_weight_sync_step = self.global_steps
    ...
    def fit(self):
        ...
        # base model chat template (R1-Zero raw mode; flag lives under azr.* — see config)
        if self.config.azr.get("pretrained_tokenizer", False):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True) and self.global_steps == 0:
            # NOTE: verl 0.7.1 AgentLoop's compute_score requires per-data_source reward fn,
            # which AZR's val data (cruxeval_i/o, livecodebench) does not have registered.
            # Initial validation is therefore skipped; set trainer.val_before_train=False
            # to make this explicit in the script.
        ...
        while self.global_steps < self.total_training_steps:
            ...
            for _ in range(self.config.azr.data_selection_strategy.update_iteration):
                ...
                with marked_timer('step', timing_raw):
                    ...
                    # concatenate batches
                    # verl 0.7.1 DataProto.concat asserts identical meta_info across sub-batches.
                    # Per-sub-batch generate populates different values for 'timing', 'global_token_num',
                    # etc., which conflict. Dynamically drop any key whose value differs across batches.
                    # NOTE: global_token_num is required by update_actor (FLOPs metric); we preserve a
                    # merged version via sum and re-inject after concat.
                    _sub_batches = list(batches.values())
                    _merged_global_token_num = None
                    if len(_sub_batches) > 1:
                        # Merge global_token_num across sub-batches (sum element-wise)
                        try:
                            _gtns = [sb.meta_info.get("global_token_num") for sb in _sub_batches]
                            _gtns = [g for g in _gtns if g is not None]
                            if _gtns:
                                # Each is typically list[int] of length world_size
                                _merged_global_token_num = [sum(vals) for vals in zip(*_gtns)]
                        except Exception:
                            _merged_global_token_num = None

                        _conflict_keys = set()
                        _ref_meta = _sub_batches[0].meta_info
                        for _sb in _sub_batches[1:]:
                            for _k, _v in _sb.meta_info.items():
                                if _k in _ref_meta and _ref_meta[_k] != _v:
                                    _conflict_keys.add(_k)
                            for _k in _ref_meta:
                                if _k not in _sb.meta_info:
                                    _conflict_keys.add(_k)
                        for _sb in _sub_batches:
                            for _k in _conflict_keys:
                                _sb.meta_info.pop(_k, None)
                    batch = DataProto.concat(_sub_batches)
                    # Re-inject merged global_token_num so update_actor's FLOPs metric works.
                    if _merged_global_token_num is not None:
                        batch.meta_info["global_token_num"] = _merged_global_token_num
                    ...
                    # NOTE: mid-training validation skipped — AZR's val data sources are not
                    # registered in verl 0.7.1's reward fn registry.
                ...
                if self.global_steps >= self.total_training_steps:
                    ...
                    # NOTE: final validation skipped — AZR's val data sources are not
                    # registered in verl 0.7.1's reward fn registry. See val_before_train note.
                    if self.config.trainer.save_freq > 0 and \
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # verl 0.7.1 AgentLoop needs raw_prompt in non_tensor_batch
            _non_tensor_pop = [k for k in ('raw_prompt', 'raw_prompt_ids') if k in test_batch.non_tensor_batch]
            ...
            # pad to be divisible by dp_size
            test_gen_batch_padded, pad_size = pad_dataproto_to_divisor(test_gen_batch, self.config.actor_rollout_ref.rollout.agent.num_workers)
