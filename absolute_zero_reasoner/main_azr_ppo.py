...
class TaskRunner:
    def run(self, config):
        ...
        if config.azr.get("pretrained_tokenizer", False):
            raw_chat_template = "{%- for message in messages -%}{{- '\n' if not loop.first -}}{{- message['content'] -}}{%- endfor -%}"
            # (1) driver tokenizer: used by RLHFDataset.__getitem__ to build input_ids.
            tokenizer.chat_template = raw_chat_template
            # (2) rollout workers / AgentLoop: verl 0.7.1's single_turn_agent_loop re-tokenizes
            # raw_prompt with the model-config tokenizer, ignoring the dataset's input_ids. Without
            # this the rollout uses Qwen3's native template -> R1 prompt double-wrapped -> garbage
            # generations -> 0 valid programs. custom_chat_template propagates to those workers.
            config.actor_rollout_ref.model.custom_chat_template = raw_chat_template
