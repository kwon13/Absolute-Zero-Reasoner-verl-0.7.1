...
class CodeIORewardManager():
    ...
    def _get_data_dict(self, data_item: DataProtoItem, problem_type: str, executor, banned_words: List[str], uid: str, banned_assertion_keywords: List[str]) -> Dict:
        ...
        _split_parts = non_special_tokens_sequences_str.split(self.splitter)
        generation = _split_parts[1].strip().strip('\"\'') if len(_split_parts) > 1 else ''
        ...
        # TEMP DEBUG: dump first few raw decodes to inspect what the live rollout produces.
        # Remove once the 0-valid-programs issue is diagnosed.
        try:
            import os as _os, json as _json
            _dbg = getattr(CodeIORewardManager, '_dbg_count', 0)
            if _dbg < 12 and _os.environ.get('AZR_DEBUG_DUMP', '1') == '1':
                CodeIORewardManager._dbg_count = _dbg + 1
                _p = _os.path.join(self.output_path or '.', '_debug_generations.jsonl')
                _os.makedirs(_os.path.dirname(_p) or '.', exist_ok=True)
                with open(_p, 'a') as _f:
                    _f.write(_json.dumps({
                        'problem_type': problem_type,
                        'splitter_found': len(_split_parts) > 1,
                        'full_sequence': non_special_tokens_sequences_str[:4000],
                        'generation_head': generation[:1500],
                        'extracted_content_head': (extracted_content or '')[:800],
                    }, ensure_ascii=False) + '\n')
        except Exception:
            pass
    ...
    def __call__(
        ...
        # AZR always computes its own reward; verl 0.7.1 AgentLoop may inject rm_scores into batch,
        # but the rm_scores there is a dummy 0 from our agent_loop _compute_score patch. Skip the early
        # return so AZR's full reward (executor + per-problem-type) runs.
    ...
    def _get_problem_generator_rewards_and_valid_programs(
        ...
        if len(valid_uids) > 0:
            ...
            temp_data = RLHFDataset(
                ...
                filter_prompts=True,
                # verl 0.7.1 AgentLoop re-tokenizes raw_prompt; must be present in the batch.
            ...
            # verl 0.7.1: keep raw_prompt/raw_prompt_ids in the batch so the AgentLoop can
            # re-tokenize; pop them alongside the tensor keys into gen_batch.
            ...
            # verl 0.7.1 hybrid mode: generate via the async AgentLoop manager. The worker-group
            # sync interface (rollout_actor_wg.generate_sequences) fails with "event loop already
            # running" because the worker process's loop is occupied by the vLLM async server.
            assert async_rollout_manager is not None, "async_rollout_manager required for difficulty estimation in verl 0.7.1"
            _divisor = len(async_rollout_manager.agent_loop_workers)
            gen_batch_padded, pad_size = pad_dataproto_to_divisor(gen_batch, _divisor)
